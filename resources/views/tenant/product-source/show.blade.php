...
@section('content')
<a href="{{ route('tenant.product-source.index') }}" class="text-sm text-mute hover:text-ink">← সব পণ্য</a>
...
<div class="grid md:grid-cols-2 gap-8 mt-4 max-w-4xl">
    ...
    <div>
        ...
        <div class="aspect-square bg-white rounded-xl border border-ink/5 grid place-items-center overflow-hidden">
            ...
            <div class="flex gap-2 mt-3 flex-wrap">
                @foreach ($images as $img)
                    <button onclick="document.getElementById('mainImage').src = '{{ asset('storage/'.$img->image_path) }}'"
                            class="w-16 h-16 rounded-lg overflow-hidden border border-ink/10 hover:border-leaf">
    ...
    <div>
        ...
        <div class="mt-6 pt-5 border-t border-ink/10 bg-white rounded-xl border border-ink/5 p-5">
            <p class="font-bold text-sm mb-3">অর্ডার করুন</p>
            @if (session('success'))
                <p class="mb-3 bg-leaf/10 border border-leaf/30 text-leafdk text-sm rounded-lg p-3">{{ session('success') }}</p>
            @endif
            @if ($errors->any())
                <p class="mb-3 bg-red-50 border border-red-200 text-red-700 text-sm rounded-lg p-3">{{ $errors->first() }}</p>
            @endif
            <form method="POST" action="{{ route('tenant.product-source.order', $product) }}" class="space-y-3">
                ...
                <div>
                    ...
                    <input name="contact_phone" required placeholder="01XXXXXXXXX" class="w-full rounded-lg border border-ink/15 px-3 py-2.5 text-sm">
                </div>
                <div>
                    <label class="text-xs text-mute">নোট (ঐচ্ছিক)</label>
                    <textarea name="note" rows="2" class="w-full rounded-lg border border-ink/15 px-3 py-2.5 text-sm"></textarea>
                </div>
                <button class="w-full py-3 rounded-lg bg-leaf text-white font-semibold text-sm hover:bg-leafdk">অর্ডার পাঠান</button>
            </form>
        </div>
    </div>
</div>
