...
<div class="flex items-center justify-between mb-6">
    ...
    <a href="{{ route('tenant.product-source.index') }}" class="text-sm text-leaf hover:underline">← পণ্য দেখুন</a>
</div>
    ...
    <table class="w-full text-sm">
        ...
        @forelse ($orders as $o)
            <tr class="border-b border-ink/5 last:border-0">
                ...
                <td class="px-4 py-3">
                    <span class="px-2 py-1 rounded text-xs {{ $o->status === 'delivered' ? 'bg-leaf/10 text-leafdk' : ($o->status === 'cancelled' ? 'bg-red-50 text-red-600' : 'bg-amber/15') }}">
        ...
        @empty
            <tr><td colspan="5" class="px-4 py-12 text-center text-mute">কোনো অর্ডার নেই।</td></tr>
        @endforelse
        </tbody>
    </table>
</div>
