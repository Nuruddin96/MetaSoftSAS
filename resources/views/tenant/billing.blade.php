...
@if (session('warning'))
    <p class="mb-4 bg-amber/15 border border-amber/40 rounded-card p-4 text-sm">{{ session('warning') }}</p>
@endif

@if (! $hasBkash && ! $hasSsl)
    <x-ui.card tone="amber" class="mb-6">
        ...
        <div class="flex flex-wrap gap-3">
            <a href="https://wa.me/{{ $supportWa }}?text={{ urlencode('আমি ' . $tenant->store_name . ' এর জন্য সাবস্ক্রিপশন পেমেন্ট করতে চাই।') }}"
               ...
               class="inline-flex items-center gap-2 px-5 py-3 rounded-btn bg-[#25D366] text-white font-semibold text-sm hover:opacity-90 transition">
            ...
            <a href="tel:{{ $supportPhone }}"
               class="inline-flex items-center gap-2 px-5 py-3 rounded-btn border border-ink/15 font-semibold text-sm hover:bg-paper transition">
...
<div class="grid md:grid-cols-3 gap-5 max-w-4xl">
    @foreach ($plans as $plan)
        <x-ui.card :class="$tenant->plan_id === $plan->id ? 'flex flex-col border-leaf ring-2 ring-leaf/20' : 'flex flex-col'">
            <div class="flex items-center justify-between">
                <h3 class="font-bold">{{ $plan->name }}</h3>
                @if ($tenant->plan_id === $plan->id)<span class="text-xs bg-leaf/10 text-leafdk px-2 py-0.5 rounded-pill font-semibold">বর্তমান</span>@endif
            </div>
                        ...
                        <div class="border border-ink/10 rounded-btn p-3">
                            ...
                            @if ($hasSsl)
                                <form method="POST" action="{{ route('tenant.billing.pay') }}">
                                    ...
                                    <x-ui.button type="submit" variant="accent" size="sm" class="w-full">
                                        পেমেন্ট করুন
                                    </x-ui.button>
                                </form>
                            ...
                            @elseif ($hasBkash)
                                <form method="POST" action="{{ route('tenant.billing.pay') }}">
                                    ...
                                    <button type="submit" class="w-full py-2.5 rounded-btn bg-[#E2136E] text-white font-semibold text-sm hover:opacity-90 transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-leaf focus-visible:ring-offset-2">bKash দিয়ে পেমেন্ট করুন</button>
                                </form>
                            @endif
                        </div>
                    @endforeach
...
@if ($payments->isNotEmpty())
    <x-ui.card padding="none" class="mt-8 max-w-4xl">
        <div class="px-5 py-3.5 border-b border-ink/5 font-bold text-sm">পেমেন্ট হিস্ট্রি</div>
        @foreach ($payments as $p)
            <div class="flex justify-between px-5 py-3 border-b border-ink/5 last:border-0 text-sm">
                ...
                <span><span class="font-semibold">{{ number_format($p->amount) }}৳</span>
                    <span class="ml-2 px-2 py-0.5 rounded-pill text-xs {{ ['completed' => 'bg-leaf/10 text-leafdk', 'failed' => 'bg-red-50 text-red-600'][$p->status] ?? 'bg-amber/15' }}">{{ $p->status }}</span></span>
            </div>
        @endforeach
    </x-ui.card>
@endif
