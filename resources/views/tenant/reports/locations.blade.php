...
<div class="grid lg:grid-cols-2 gap-6">
    <x-ui.card padding="none">
        <div class="px-5 py-3.5 border-b border-ink/5 font-bold text-sm">বিভাগ অনুযায়ী</div>
        @forelse ($byDivision as $d)
            <div class="px-5 py-3 border-b border-ink/5 last:border-0">
                ...
                <div class="h-2 rounded-pill bg-ink/5 overflow-hidden">
                    <div class="h-full bg-leaf rounded-pill" style="width: {{ round($d->orders / $maxDiv * 100) }}%"></div>
                </div>
            </div>
        @empty
            <p class="px-5 py-10 text-center text-mute text-sm">এই সময়ে কোনো অর্ডার নেই।</p>
        @endforelse
    </x-ui.card>

    <x-ui.card padding="none">
        <div class="px-5 py-3.5 border-b border-ink/5 font-bold text-sm">শীর্ষ জেলা</div>
