    ...
    <form method="POST" action="{{ route('tenant.website.brand') }}" enctype="multipart/form-data">
        ...
        <div>
            ...
            <div class="mt-2 flex items-center gap-4">
                <div class="w-20 h-20 rounded-card border border-ink/10 bg-paper grid place-items-center overflow-hidden shrink-0">
                    @if ($tenant->logo_path)
            ...
            <div>
                ...
                <div class="mt-1 flex gap-2">
                    <input type="color" name="primary_color" id="primaryPicker" value="{{ $tenant->primary_color ?? '#128155' }}"
                           class="h-11 w-14 rounded-btn border border-ink/15 cursor-pointer">
                    <input value="{{ $tenant->primary_color ?? '#128155' }}" readonly
                           class="flex-1 rounded-btn border border-ink/15 px-3 text-sm bg-paper" id="pcHex">
                </div>
            ...
            <div>
                ...
                <div class="mt-1 flex gap-2">
                    ...
                    <input value="{{ $tenant->secondary_color ?? '#f59e0b' }}" readonly
                           class="flex-1 rounded-btn border border-ink/15 px-3 text-sm bg-paper" id="scHex">
                </div>
            ...
            <div class="flex flex-wrap gap-2">
                ...
                @foreach ($presets as [$label, $c1, $c2])
                    <button type="button" onclick="applyPreset('{{ $c1 }}', '{{ $c2 }}')"
                            title="{{ $label }}"
                            class="w-10 h-10 rounded-full border-2 border-white shadow ring-1 ring-ink/10 hover:ring-leaf hover:scale-110 transition overflow-hidden focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-leaf focus-visible:ring-offset-2">
        ...
        <div class="flex gap-3">
            ...
            @if ($tenant->logo_path)
                <a href="{{ route('tenant.website.logo.remove') }}"
                   ...
                   class="px-4 py-2.5 rounded-btn border border-ink/15 text-sm hover:bg-paper transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-leaf focus-visible:ring-offset-2">লোগো সরান</a>
            @endif
        </div>
        </x-ui.card>
    </form>
...
<section data-panel="home" class="hidden max-w-3xl space-y-6">
    <x-ui.card>
        <form method="POST" action="{{ route('tenant.website.homepage') }}" class="space-y-5">
        @csrf
        <p class="font-bold">হোম পেজের সাজসজ্জা</p>

        <div>
            <label class="text-sm font-medium">ব্যানার স্টাইল</label>
            <select name="hero_style" class="mt-1 w-full rounded-btn border border-ink/15 px-3 py-2.5 bg-white">
    ...
    <x-ui.card>
        ...
        <div class="space-y-3 mb-5">
            @forelse ($banners as $b)
                <div class="flex items-center gap-4 border border-ink/10 rounded-btn p-3">
                    <img src="{{ asset('storage/' . $b->image_path) }}" class="w-28 h-16 object-cover rounded-lg shrink-0" loading="lazy">
            ...
            @empty
                <p class="text-sm text-mute text-center py-6 border border-dashed border-ink/15 rounded-btn">কোনো ব্যানার নেই</p>
            @endforelse
        </div>
        ...
        <form method="POST" action="{{ route('tenant.website.banner.store') }}" enctype="multipart/form-data" class="space-y-3 border-t border-ink/10 pt-5">
            ...
            <div class="grid md:grid-cols-2 gap-3">
                ...
                <input name="button_text" placeholder="বাটনের লেখা (যেমন: এখনই কিনুন)" class="rounded-btn border border-ink/15 px-3 py-2.5 text-sm">
                <input name="button_link" placeholder="বাটনের লিংক (যেমন: /products)" class="rounded-btn border border-ink/15 px-3 py-2.5 text-sm">
            </div>
            <x-ui.button type="submit" variant="primary" size="sm">+ ব্যানার যোগ করুন</x-ui.button>
        </form>
    </x-ui.card>
</section>
...
<section data-panel="pages" class="hidden max-w-3xl space-y-6">
    <x-ui.card padding="none">
        <div class="px-5 py-3.5 border-b border-ink/5 font-bold text-sm">আপনার পেজগুলো</div>
        @forelse ($pages as $p)
            <div class="flex items-center justify-between px-5 py-3 border-b border-ink/5 last:border-0">
                ...
                <div class="flex items-center gap-3">
                    <a href="{{ route('tenant.website.page.edit', $p) }}" class="text-leaf text-xs hover:underline rounded focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-leaf focus-visible:ring-offset-2">এডিট</a>
        ...
        @empty
            <p class="px-5 py-10 text-center text-mute text-sm">কোনো পেজ নেই। নিচে থেকে তৈরি করুন — যেমন "আমাদের সম্পর্কে", "রিটার্ন পলিসি"।</p>
        @endforelse
    </x-ui.card>

    <x-ui.card class="space-y-4">
        <form method="POST" action="{{ route('tenant.website.page.store') }}" class="space-y-4">
        @csrf
        <p class="font-bold">নতুন পেজ তৈরি করুন</p>
        <input name="title" required placeholder="পেজের নাম (যেমন: আমাদের সম্পর্কে)"
               class="w-full rounded-btn border border-ink/15 px-3 py-2.5 focus:ring-2 focus:ring-leaf outline-none">
        ...
        <x-ui.button type="submit" variant="accent" size="sm">পেজ তৈরি করুন</x-ui.button>
        </form>
    </x-ui.card>

    <x-ui.card tone="amber" class="text-sm">
        ...
        <div class="border-t border-ink/10 pt-5">
            ...
            <div class="grid md:grid-cols-2 gap-3">
                <input name="social_facebook" value="{{ $set['social_facebook'] ?? '' }}" placeholder="https://facebook.com/yourpage"
                       class="rounded-btn border border-ink/15 px-3 py-2.5 text-sm">
    ...
    document.querySelectorAll('.tab-btn').forEach(function (btn) {
        btn.addEventListener('click', function () {
            document.querySelectorAll('.tab-btn').forEach(function (b) {
                b.className = 'tab-btn px-4 py-2 rounded-btn text-sm font-semibold transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-leaf focus-visible:ring-offset-2 bg-white border border-ink/10 hover:border-leaf/40';
            });
            btn.className = 'tab-btn px-4 py-2 rounded-btn text-sm font-semibold transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-leaf focus-visible:ring-offset-2 bg-ink text-white';
